...
def login():
	...
	try:
		...
		if login_user(ret_user,force=True):
			if 'redirect_url' in data and data['redirect_url'][-6:]!='signup':
	...
	if request.method=='POST':
		...
		try:
			...
			for subject in subjects:
				if len(subject)>1:
					ok=True
					verified_subjects.append(subject.lower().strip())
...
def user_queries():
	client=MongoClient()
	db=client.local_tutor
	searches_db=db.searches.find()
	searches=[]
	for search in searches_db:
